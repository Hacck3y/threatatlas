...
The **scenarios** API is a PRO-only, job-queued surface on top of the ThreatAtlas chokepoint + trade dataset. Callers enqueue a named scenario template against an optional country, then poll a job-id until the worker completes.

<Info>
This service is documented inline (not yet proto-backed). Proto migration is tracked in [issue #3207](https://github.com/hacck3y/threatatlas/issues/3207) and will replace this page with auto-generated reference.
</Info>
...
**Response** — abbreviated example using one of the live shipped templates (`server/threatatlas/supply-chain/v1/scenario-templates.ts`):
```json
{
  "templates": [
    {
