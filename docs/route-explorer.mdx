...
Route Explorer is available by default in ThreatAtlas. The workflow opens directly into the full lane, impact, and chokepoint tabs without any access gate.

## Data behind Route Explorer

- **Route graph** — `/api/supply-chain/v1/get-route-explorer-lane` (generated from `proto/threatatlas/supply_chain/v1/`). Returns primary route id, chokepoint exposures, bypass corridors, war-risk tier, and disruption score.
