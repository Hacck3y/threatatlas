...
- **Cloud fallback** — when a local API handler fails or is missing, requests transparently fall through to the cloud deployment (threatatlas.app) with origin headers stripped
...
ThreatAtlas desktop uses a runtime configuration schema with per-feature toggles and secret-backed credentials.
...
- ThreatAtlas API key: cloud fallback is blocked; desktop operates local-only.
...
The desktop app checks for new versions by polling `threatatlas.app/api/version` — once at startup (after a 5-second delay) and then every 6 hours. When a newer version is detected (semver comparison), a non-intrusive update badge appears with a direct link to the GitHub Release page.
