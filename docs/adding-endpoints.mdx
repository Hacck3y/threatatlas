...
description: "All JSON API endpoints in ThreatAtlas must use sebuf. This guide walks through adding a new RPC to an existing service and adding an entirely new service."
---
All JSON API endpoints in ThreatAtlas **must** use sebuf. Do not create standalone `api/*.js` files — the legacy pattern is deprecated and being removed.

This guide walks through adding a new RPC to an existing service and adding an entirely new service.

> **Important:** After modifying any `.proto` file, you **must** run `make generate` before building or pushing. The generated TypeScript files in `src/generated/` are checked into the repo and must stay in sync with the proto definitions. CI does not run generation yet — this is your responsibility until we add it to the pipeline (see [#200](https://github.com/hacck3y/threatatlas/issues/200)).
...
Create `proto/threatatlas/seismology/v1/get_earthquake_details.proto`:

```protobuf
syntax = "proto3";
package threatatlas.seismology.v1;

import "buf/validate/validate.proto";
import "threatatlas/seismology/v1/earthquake.proto";
...
Edit `proto/threatatlas/seismology/v1/service.proto`:

```protobuf
import "threatatlas/seismology/v1/get_earthquake_details.proto";
...
Create `server/threatatlas/seismology/v1/get-earthquake-details.ts`:
...
Edit `server/threatatlas/seismology/v1/handler.ts`:
...
```
proto/threatatlas/sanctions/v1/
```

### 2. Define entity messages

Create `proto/threatatlas/sanctions/v1/sanctions_entry.proto`:

```protobuf
syntax = "proto3";
package threatatlas.sanctions.v1;
...
Create `proto/threatatlas/sanctions/v1/list_sanctions.proto`:
...
import "buf/validate/validate.proto";
import "threatatlas/core/v1/pagination.proto";
import "threatatlas/sanctions/v1/sanctions_entry.proto";
...
message ListSanctionsRequest {
  ...
  threatatlas.core.v1.PaginationRequest pagination = 3;
}
...
message ListSanctionsResponse {
  ...
  // Pagination metadata.
  threatatlas.core.v1.PaginationResponse pagination = 2;
}
```

### 4. Define the service

Create `proto/threatatlas/sanctions/v1/service.proto`:
...
import "sebuf/http/annotations.proto";
import "threatatlas/sanctions/v1/list_sanctions.proto";
...
```
server/threatatlas/sanctions/v1/
├── handler.ts               # thin re-export
└── list-sanctions.ts        # RPC implementation
```

`server/threatatlas/sanctions/v1/list-sanctions.ts`:
```typescript
import type {
...
`server/threatatlas/sanctions/v1/handler.ts`:
```typescript
...
| Type | Import | Use for |
|------|--------|---------|
| `GeoCoordinates` | `threatatlas/core/v1/geo.proto` | Any lat/lon location (has built-in -90/90 and -180/180 bounds) |
| `BoundingBox` | `threatatlas/core/v1/geo.proto` | Spatial filtering |
| `TimeRange` | `threatatlas/core/v1/time.proto` | Time-based filtering (has `INT64_ENCODING_NUMBER`) |
| `PaginationRequest` | `threatatlas/core/v1/pagination.proto` | Request pagination (has page_size 1-100 constraint) |
| `PaginationResponse` | `threatatlas/core/v1/pagination.proto` | Response pagination metadata |
