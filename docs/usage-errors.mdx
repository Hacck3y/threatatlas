...
- Every edge response includes `x-vercel-id` and `x-threatatlas-deploy` headers — include those when reporting issues.
- Sentry alerts forward to [status.threatatlas.app](https://status.threatatlas.app/).
