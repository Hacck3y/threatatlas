...
description: "This guide provides reproducible local packaging steps for both desktop variants: full (ThreatAtlas) and tech (Tech Monitor)."
---
...
- Full variant: `ThreatAtlas` / `world-monitor`
- Tech variant: `Tech Monitor` / `tech-monitor`

Distinct names are configured in Tauri:

- `src-tauri/tauri.conf.json` → `ThreatAtlas` / `world-monitor`
