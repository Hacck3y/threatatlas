...
ThreatAtlas has **three** authentication modes. Which one applies depends on how you're calling.

## Auth matrix

| Mode | Header | Used by | Trusted on which endpoints? |
|------|--------|---------|------------------------------|
| **Browser origin** | `Origin: https://www.threatatlas.app` (browser-set) | Dashboard, desktop app | Most public endpoints — but **not** `forceKey: true` routes. |
| **API key** | `X-ThreatAtlas-Key: wm_live_...` | Server-to-server, scripts, SDKs | All endpoints, including `forceKey: true`. |
| **OAuth bearer** | `Authorization: Bearer <oauth-token>` | MCP clients (Claude, Cursor, Inspector) | `/api/mcp`. The handler also accepts a direct `X-ThreatAtlas-Key` in lieu of an OAuth token — see [MCP](/mcp#authentication). |
...
For these, you **must** send `X-ThreatAtlas-Key`.
...
<Warning>
**A Cloudflare Worker** (`api-cors-preflight`) is the authoritative CORS handler for `api.threatatlas.app` — it overrides `_cors.js` and `vercel.json`. If you're changing origin rules, change them in the Cloudflare dashboard.
</Warning>
...
```
X-ThreatAtlas-Key: wm_live_abcdef0123456789...
```
...
2. Else, check `X-ThreatAtlas-Key` against `WORLDMONITOR_VALID_KEYS` (env).
...
- A valid `X-ThreatAtlas-Key` (env-allowlisted from `WORLDMONITOR_VALID_KEYS`, or a user-owned `wm_`-prefixed key whose Convex record has the `apiAccess` entitlement), **or**
