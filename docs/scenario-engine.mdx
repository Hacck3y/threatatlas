...
Templates are defined in `server/threatatlas/supply-chain/v1/scenario-templates.ts`. Each template has a `type` drawn from a small, curated set so scenarios are browsable by category rather than a free-form list.
...
- **Scenario templates** — `server/threatatlas/supply-chain/v1/scenario-templates.ts`. Additions require a proto-side change; not user-configurable today.
