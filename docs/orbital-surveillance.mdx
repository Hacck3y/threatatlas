...
| Feature | Core | Planned | Enterprise |
|---------|------|---------|------------|
| Live satellite positions on globe | Yes | — | Yes |
| Orbit trails (15-min trace) | Yes | — | Yes |
| Ground footprint markers | Yes | — | Yes |
...
Push notifications when SAR or optical satellites are overhead a user's watched regions. Integrates with future delivery channels (Slack, Telegram, WhatsApp, Email).
...
Dedicated satellite intel panel with a table of tracked satellites: operator, sensor capability, orbit type, current position, and next pass over user-defined points of interest.

### Historical Pass Log

Which satellites passed over a given location in the last 24h (planned) or 30 days (Enterprise). Useful for post-event analysis: _"What imaging assets were overhead during the incident?"_
