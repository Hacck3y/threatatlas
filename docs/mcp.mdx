...
| Endpoint | Purpose |
|----------|---------|
| `https://api.threatatlas.app/api/mcp` | JSON-RPC server (Streamable HTTP transport, protocol `2025-03-26`) |
| `https://api.threatatlas.app/api/oauth/register` | Dynamic Client Registration (RFC 7591) |
| `https://api.threatatlas.app/api/oauth/authorize` | OAuth 2.1 authorization endpoint (PKCE required) |
| `https://api.threatatlas.app/api/oauth/token` | Token endpoint (authorization_code + refresh_token) |
| `https://api.threatatlas.app/.well-known/oauth-authorization-server` | AS metadata (RFC 8414) |
| `https://api.threatatlas.app/.well-known/oauth-protected-resource` | Resource server metadata (RFC 9728) |

Protocol version: `2025-03-26`. Server identifier: `threatatlas` v1.0.
...
2. **Direct API key** — `X-ThreatAtlas-Key: wm_live_...`. Intended for server-side scripts, `curl`, and custom integrations. Do **not** send a `wm_live_...` key as a `Bearer` token — it will fail OAuth resolution and return `401 invalid_token`.
...
```json
{
  "mcpServers": {
    "threatatlas": {
      "url": "https://api.threatatlas.app/api/mcp"
    }
  }
}
```
...
```bash
npx @modelcontextprotocol/inspector https://api.threatatlas.app/api/mcp
```
...
Server-side with a direct API key — send it as `X-ThreatAtlas-Key`, **not** as a bearer token.

```bash
WM_KEY="wm_live_..."

# 1. List tools
curl -s https://api.threatatlas.app/api/mcp \
  -H "X-ThreatAtlas-Key: $WM_KEY" \
...
Seed-level health per key: [status.threatatlas.app](https://status.threatatlas.app/).
...
- [ThreatAtlas dashboard](https://www.threatatlas.app)
