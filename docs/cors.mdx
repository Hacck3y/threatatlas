...
| Pattern | Matches |
| --- | --- |
| `(*.)?threatatlas.app` | Production + subdomains (`tech.`, `finance.`, etc.) |
| `threatatlas-*-elie-*.vercel.app` | Vercel preview deploys |
...
- `Content-Type`
- `Authorization`
- `X-ThreatAtlas-Key` (API key for desktop/third-party access). See [API Key Gating](/api-key-deployment) for key management details.
