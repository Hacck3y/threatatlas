...
|---|---|---|---|---|
| `resilience:score:v9:{countryCode}` | JSON | 6 hours | `buildResilienceScore` in `server/threatatlas/resilience/v1/_shared.ts` | `getResilienceScore` handler |
...
**Previous published version.** Phase 1 of the reference-grade upgrade. Methodology surface reorganized for full reproducibility without changing the top-line domain weights or scoring formula.
...
| **Source quality** | 7.0 | World Bank, IMF, WHO, IEA, UNHCR, UCDP, IPC, BIS, FAO, RSF, GPI: all authoritative. Gap: curated-list sources (BIS ~40 economies, WTO) do not cover the full ThreatAtlas country set, which is why the `unmonitored` imputation class exists. Phase 2 T2.9 adds language-normalized information signal to reduce English-press bias. |
