...
description: "Bootstrap, health, versioning, cache-purge, and user preferences — the plumbing endpoints every ThreatAtlas client talks to."
---
...
- **Auth**: browser origin OR `X-ThreatAtlas-Key`
...
Returns the latest **GitHub Release** of `hacck3y/threatatlas`. Used by the desktop app to detect a newer published release and prompt the user to update. It is **not** the currently-deployed Vercel commit.

```json
{
  "version": "2.6.7",
  "tag": "v2.6.7",
  "url": "https://github.com/hacck3y/threatatlas/releases/tag/v2.6.7",
  "prerelease": false
}
```
...
Redirects to the matching asset on the latest GitHub release of `hacck3y/threatatlas`. Returns `302` to the asset URL on success, or `302` to [releases/latest](https://github.com/hacck3y/threatatlas/releases/latest) on any failure (unknown platform, no match, GitHub error).
