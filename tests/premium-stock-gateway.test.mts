...
describe('premium gateway API key enforcement', () => {
  it('requires credentials for premium endpoints regardless of origin', async () => {
    ...
    const browserNoKey = await handler(new Request('https://threatatlas.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: { Origin: 'https://threatatlas.app' },
    }));
    assert.equal(browserNoKey.status, 401);

    const resilienceScoreNoKey = await handler(new Request('https://threatatlas.app/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: { Origin: 'https://threatatlas.app' },
    }));
    assert.equal(resilienceScoreNoKey.status, 401);

    const resilienceRankingNoKey = await handler(new Request('https://threatatlas.app/api/resilience/v1/get-resilience-ranking', {
      headers: { Origin: 'https://threatatlas.app' },
    }));
    ...
    const browserWithKey = await handler(new Request('https://threatatlas.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
    ...
    const resilienceScoreWithKey = await handler(new Request('https://threatatlas.app/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: {
        Origin: 'https://threatatlas.app',
        'X-WorldMonitor-Key': 'real-key-123',
      },
    ...
    const resilienceRankingWithKey = await handler(new Request('https://threatatlas.app/api/resilience/v1/get-resilience-ranking', {
      headers: {
        Origin: 'https://threatatlas.app',
        'X-WorldMonitor-Key': 'real-key-123',
      },
    ...
    const publicAllowed = await handler(new Request('https://threatatlas.app/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: { Origin: 'https://threatatlas.app' },
    }));
...
describe('premium gateway bearer token auth', () => {
  ...
  it('valid bearer token resolves userId but entitlement check still applies', async () => {
    ...
    const res = await handler(new Request('https://threatatlas.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
        Origin: 'https://threatatlas.app',
  ...
  it('public routes are unaffected by absence of auth header', async () => {
    const res = await handler(new Request('https://threatatlas.app/api/market/v1/list-market-quotes?symbols=AAPL', {
      headers: { Origin: 'https://threatatlas.app' },
    }));
  ...
  it('rejects free bearer token on resilience premium endpoints → 403', async () => {
    ...
    const scoreRes = await handler(new Request('https://threatatlas.app/api/resilience/v1/get-resilience-score?countryCode=US', {
      headers: {
        Origin: 'https://threatatlas.app',
        Authorization: `Bearer ${token}`,
      },
    ...
    const rankingRes = await handler(new Request('https://threatatlas.app/api/resilience/v1/get-resilience-ranking', {
      headers: {
        Origin: 'https://threatatlas.app',
        Authorization: `Bearer ${token}`,
      },
