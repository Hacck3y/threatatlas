...
describe('getHumanitarianSummary handler', () => {
  const src = readSrc('server/threatatlas/conflict/v1/get-humanitarian-summary.ts');
...
describe('LLM prompt political context (LOW-1)', () => {
  const src = readSrc('server/threatatlas/news/v1/_shared.ts');
...
describe('getCacheKey determinism', () => {
  ...
  const sharedSrc = readSrc('server/threatatlas/news/v1/_shared.ts');
...
describe('getVesselSnapshot caching (HIGH-1)', () => {
  const src = readSrc('server/threatatlas/maritime/v1/get-vessel-snapshot.ts');
...
describe('getSimulationOutcome handler', () => {
  const src = readSrc('server/threatatlas/forecast/v1/get-simulation-outcome.ts');
