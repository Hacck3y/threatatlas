...
describe('deploy/cache configuration guardrails', () => {
  it('disables caching for HTML entry routes on Vercel', () => {
    const spaNoCache = getCacheHeaderValue('/((?!api|mcp|oauth|assets|blog|docs|favico|map-styles|data|textures|sw\\.js|workbox-[a-f0-9]+\\.js|manifest\\.webmanifest|offline\\.html|robots\\.txt|sitemap\\.xml|llms\\.txt|llms-full\\.txt|\\.well-known|wm-widget-sandbox\\.html).*)');
...
describe('security header guardrails', () => {
  ...
  it('CSP script-src omits Clerk origin after sign-in UI removal', () => {
    const csp = getHeaderValue('Content-Security-Policy');
    const scriptSrc = csp.match(/script-src\s+([^;]+)/)?.[1] ?? '';
    assert.ok(!scriptSrc.includes('clerk.'), 'CSP script-src should not include Clerk auth origins');
  });

  it('CSP frame-src omits Clerk origin after sign-in UI removal', () => {
    const csp = getHeaderValue('Content-Security-Policy');
    const frameSrc = csp.match(/frame-src\s+([^;]+)/)?.[1] ?? '';
    assert.ok(!frameSrc.includes('clerk.'), 'CSP frame-src should not include Clerk auth origins');
  });
