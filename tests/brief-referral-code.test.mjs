...
describe('buildShareUrl', () => {
  it('appends /pro?ref={code} to the base URL', () => {
    assert.equal(
      buildShareUrl('https://threatatlas.app', 'abc12345'),
      'https://threatatlas.app/pro?ref=abc12345',
    );
  });
  ...
  it('URL-encodes the code (defensive — code is always hex in practice)', () => {
    assert.equal(
      buildShareUrl('https://threatatlas.app', 'a b'),
      'https://threatatlas.app/pro?ref=a%20b',
    );
  });
});
