...
async function requestPublicRoute(origin: string) {
  ...
  return handler(new Request('https://threatatlas.app/api/market/v1/list-market-quotes?symbols=AAPL', {
    headers: { Origin: origin },
  }));
}

describe('gateway CDN origin policy', () => {
  it('keeps per-origin CORS and enables CDN caching for threatatlas.app', async () => {
    const res = await requestPublicRoute('https://threatatlas.app');
    assert.equal(res.status, 200);
    assert.equal(res.headers.get('Access-Control-Allow-Origin'), 'https://threatatlas.app');
  ...
  it('keeps per-origin CORS and enables CDN caching for production subdomains', async () => {
    const res = await requestPublicRoute('https://tech.threatatlas.app');
    assert.equal(res.status, 200);
    assert.equal(res.headers.get('Access-Control-Allow-Origin'), 'https://tech.threatatlas.app');
    assert.equal(res.headers.get('Vary'), 'Origin');
    assert.match(res.headers.get('CDN-Cache-Control') ?? '', /s-maxage=/);
  });
  ...
  it('preserves premium auth behavior', async () => {
    ...
    const noCreds = await handler(new Request('https://threatatlas.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: { Origin: 'https://threatatlas.app' },
    }));
    assert.equal(noCreds.status, 401);

    const withKey = await handler(new Request('https://threatatlas.app/api/market/v1/analyze-stock?symbol=AAPL', {
      headers: {
    ...
    assert.equal(withKey.headers.get('Access-Control-Allow-Origin'), 'https://threatatlas.app');
