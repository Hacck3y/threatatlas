...
describe('widget-agent relay — security', () => {
  ...
  it('SSRF guard — allowlist is checked before any fetch call in tool loop', () => {
    ...
    // The fetch call to api.threatatlas.app must come AFTER the check
    const fetchCallIdx = relay.indexOf("'https://api.threatatlas.app'", allowlistCheck);
    assert.ok(
      fetchCallIdx > allowlistCheck,
      'fetch() to api.threatatlas.app must appear after allowlist check',
    );
  });
  ...
  it('CORS reuses getCorsOrigin (not a narrow hardcoded origin list)', () => {
    ...
    assert.ok(
      !corsBlock.includes("['https://threatatlas.app'"),
...
describe('widget-store — constants and logic', () => {
  ...
  it('deleteWidget cleans threatatlas-panel-spans (aggregate map)', () => {
    assert.ok(
      store.includes("'threatatlas-panel-spans'"),
      "deleteWidget must clean 'threatatlas-panel-spans'",
    );
  });

  it('deleteWidget cleans threatatlas-panel-col-spans (aggregate map)', () => {
    assert.ok(
      store.includes("'threatatlas-panel-col-spans'"),
      "deleteWidget must clean 'threatatlas-panel-col-spans'",
    );
  });
...
describe('proxy routing — widgetAgentUrl', () => {
  ...
  it('widgetAgentUrl targets proxy.threatatlas.app (not toRuntimeUrl)', () => {
    // The URL may be in a constant above the function; search the whole file
    assert.ok(
      proxy.includes('proxy.threatatlas.app'),
      'Must target proxy.threatatlas.app directly (sidecar destroys SSE via arrayBuffer)',
    );
  ...
  it('vite.config.ts proxies /widget-agent to proxy.threatatlas.app', () => {
    assert.ok(
      vite.includes('/widget-agent'),
      'vite.config.ts must have proxy entry for /widget-agent',
    );
    assert.ok(
      vite.includes('proxy.threatatlas.app'),
      'Vite proxy target must be proxy.threatatlas.app',
    );
  });
