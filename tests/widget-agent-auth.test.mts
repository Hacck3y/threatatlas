...
describe('widget-agent unified tester key auth', () => {
  ...
  it('accepts X-WorldMonitor-Key and upgrades relay request to pro', async () => {
    const res = await handler(new Request('https://www.threatatlas.app/api/widget-agent', {
      method: 'POST',
      headers: {
        Origin: 'https://www.threatatlas.app',
    ...
    assert.equal(call.arguments[0], 'https://proxy.threatatlas.app/widget-agent');
