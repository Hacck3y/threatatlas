...
function makeSwSandbox() {
  ...
  const self = {
    location: { origin: 'https://threatatlas.app' },
...
describe('push-handler.js — push event', () => {
  it('renders a notification with the payload fields', () => {
    ...
    box.emit('push', pushEvent({
      ...
      url: 'https://threatatlas.app/api/brief/user_abc/2026-04-18?t=xxx',
    ...
    assert.equal(opts.data.url, 'https://threatatlas.app/api/brief/user_abc/2026-04-18?t=xxx');
...
describe('push-handler.js — notificationclick', () => {
  it('opens the target url when no existing window matches', async () => {
    const box = makeSwSandbox();
    loadHandlerInto(box);
    const ev = notifClickEvent({ url: 'https://threatatlas.app/api/brief/user_a/2026-04-18?t=abc' });
    box.emit('notificationclick', ev);
    assert.equal(ev.closed, true);
    // Wait for the waitUntil chain
    for (const p of ev.waits) await p;
    assert.equal(box.opened, 'https://threatatlas.app/api/brief/user_a/2026-04-18?t=abc');
  });
  ...
  it('focuses + navigates an existing same-origin window instead of opening', async () => {
    ...
    box.windowClients.push({
      url: 'https://threatatlas.app/',
      focus() { focused = true; return this; },
      navigate(url) { navigated = url; return Promise.resolve(); },
    });
    loadHandlerInto(box);
    const ev = notifClickEvent({ url: 'https://threatatlas.app/api/brief/u/d?t=t' });
    box.emit('notificationclick', ev);
    for (const p of ev.waits) await p;
    assert.equal(focused, true);
    assert.equal(navigated, 'https://threatatlas.app/api/brief/u/d?t=t');
