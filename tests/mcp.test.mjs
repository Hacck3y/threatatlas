...
const BASE_URL = 'https://api.threatatlas.app/mcp';
...
describe('api/mcp.ts — PRO MCP Server', () => {
  ...
  it('returns JSON-RPC -32001 when invalid API key provided', async () => {
    const req = makeReq('POST', initBody(), { 'X-ThreatAtlas-Key': 'wrong_key' });
    const res = await handler(req);
  ...
  it('OPTIONS returns 204 with CORS headers', async () => {
    const req = new Request(BASE_URL, { method: 'OPTIONS', headers: { origin: 'https://threatatlas.app' } });
    const res = await handler(req);
  ...
  it('malformed body returns JSON-RPC -32600', async () => {
    const req = new Request(BASE_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'X-ThreatAtlas-Key': VALID_KEY },
      body: '{bad json',
    });
    const res = await handler(req);
    const body = await res.json();
