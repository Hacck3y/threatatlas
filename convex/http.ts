  ...
  handler: httpAction(async (ctx, request) => {
    ...
    if (claimed.ok && botToken) {
      await fetch(`https://api.telegram.org/bot${botToken}/sendMessage`, {
        method: "POST",
        headers: { "Content-Type": "application/json", "User-Agent": "threatatlas-convex/1.0" },
        body: JSON.stringify({
          chat_id: chatId,
          text: "ThreatAtlas connected. You'll receive breaking news alerts here.",
        }),
...
// shared referral links can credit the sharer via the userReferralCredits
// path in registerInterest:register. Auth is
...
// Signature verification + internal mutation.
http.route({
  path: "/resend-webhook",
  method: "POST",
