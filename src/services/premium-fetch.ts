...
function reportServerError(res: Response, input: RequestInfo | URL): void {
  ...
  try {
    ...
    const path = new URL(href, globalThis.location?.href ?? 'https://threatatlas.app').pathname;
...
export async function premiumFetch(
  ...
  if (existing.has('Authorization') || existing.has('X-ThreatAtlas-Key')) {
  ...
  try {
    ...
    if (wmKey) {
      existing.set('X-ThreatAtlas-Key', wmKey);
  ...
  for (const testerKey of testerKeys) {
    ...
    testerHeaders.set('X-ThreatAtlas-Key', testerKey);
