...
const DEFAULT_WEB_API_URL = 'https://api.threatatlas.app';
...
function isWorldMonitorWebHost(hostname: string): boolean {
  return hostname === 'threatatlas.app'
    || hostname === 'www.threatatlas.app'
    || hostname.endsWith('.threatatlas.app');
}
...
export function getRemoteApiBaseUrl(): string {
  ...
  if (isDesktopRuntime()) return 'https://threatatlas.app';
  return '';
}
...
function isAppOriginUrl(urlStr: string): boolean {
  try {
    ...
    return APP_HOSTS.has(host) || host.endsWith('.threatatlas.app');
  } catch {
    return false;
  }
}
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const cloudFallback = async () => {
      ...
      if (KEYED_CLOUD_API_PATTERN.test(target)) {
        ...
        if (wmKeyValue) {
          cloudHeaders.set('X-ThreatAtlas-Key', wmKeyValue);
        }
      }
...
export function installWebApiRedirect(): void {
   ...
   *   2. WORLDMONITOR_API_KEY from runtime config → X-ThreatAtlas-Key
   *   3. Tester key (wm-pro-key / wm-widget-key) → X-ThreatAtlas-Key
  ...
  const enrichInitForPremium = async (pathWithQuery: string, init?: RequestInit): Promise<RequestInit | undefined> => {
    ...
    if (testerKey) {
      headers.set('X-ThreatAtlas-Key', testerKey);
      return { ...init, headers };
    }
  ...
  if (hasRedirect) {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
      if (typeof input === 'string') {
        ...
        // with full URLs like https://api.threatatlas.app/api/...) — just inject auth.
