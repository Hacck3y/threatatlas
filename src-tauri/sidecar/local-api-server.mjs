...
async function proxyToCloud(requestUrl, req, remoteBase) {
  ...
  headers.set('Origin', 'https://threatatlas.app');
  return fetch(target, {
    method: req.method,
    headers,
    body,
...
function resolveConfig(options = {}) {
  ...
  const remoteBase = String(options.remoteBase ?? process.env.LOCAL_API_REMOTE_BASE ?? 'https://api.threatatlas.app').replace(/\/$/, '');
  ...
  if (mode === 'docker' && requestedFallback) {
    (options.logger ?? console).warn('[local-api] Cloud fallback disabled in Docker mode (self-hosted instances must not proxy to api.threatatlas.app)');
  }
...
const SIDECAR_ALLOWED_ORIGINS = [
  ...
  // Only allow exact domain or single-level subdomains (e.g. preview-xyz.threatatlas.app).
  // The previous (.*\.)? pattern was overly broad. Anchored to prevent spoofing
  // via domains like threatatlasEVIL.vercel.app.
  /^https:\/\/([a-z0-9-]+\.)?threatatlas\.app$/,
];
