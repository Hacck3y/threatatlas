...
test.describe('theme toggle (happy variant)', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.addInitScript(() => {
      if (!sessionStorage.getItem('__test_init_done')) {
        localStorage.removeItem('threatatlas-theme');
        localStorage.removeItem('threatatlas-variant');
  ...
  test('dark mode persists across page reload', async ({ page }) => {
    ...
    const stored = await page.evaluate(() => localStorage.getItem('threatatlas-theme'));
  ...
  test('no FOUC: data-theme is set before main CSS loads', async ({ page }) => {
    ...
    await page.addInitScript(() => {
      localStorage.setItem('threatatlas-theme', 'dark');
      localStorage.setItem('threatatlas-variant', 'happy');
    });
