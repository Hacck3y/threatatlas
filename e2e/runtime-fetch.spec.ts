...
test.describe('desktop runtime routing guardrails', () => {
    ...
    const result = await page.evaluate(async () => {
      ...
      return {
        ...
        webHost: runtime.detectDesktopRuntime({
          ...
          userAgent: 'Mozilla/5.0',
          locationProtocol: 'https:',
          locationHost: 'threatatlas.app',
          locationOrigin: 'https://threatatlas.app',
        }),
      };
    });
  ...
  test('runtime fetch patch falls back to cloud for local failures', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('threatatlas.app/api/fred-data')) {
        ...
        if (url.includes('threatatlas.app/api/stablecoin-markets')) {
      ...
      await runtimeConfig.setSecretValue('THREATATLAS_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, 'ta_test_key_1234567890abcdef');
    ...
    expect(result.calls.some((url) => url.includes('threatatlas.app/api/fred-data'))).toBe(true);
    expect(result.calls.some((url) => url.includes('127.0.0.1:46123/api/stablecoin-markets'))).toBe(true);
    expect(result.calls.some((url) => url.includes('threatatlas.app/api/stablecoin-markets'))).toBe(true);
  });
  ...
  test('runtime fetch patch never sends local-only endpoints to cloud', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL) => {
        ...
        if (url.includes('threatatlas.app/api/local-env-update')) {
          return responseJson({ leaked: true }, 200);
        }
        if (url.includes('threatatlas.app/api/local-validate-secret')) {
          return responseJson({ leaked: true }, 200);
        }
    ...
    expect(result.calls.some((url) => url.includes('threatatlas.app/api/local-env-update'))).toBe(false);
    expect(result.calls.some((url) => url.includes('threatatlas.app/api/local-validate-secret'))).toBe(false);
  });
  ...
  test('update badge picks architecture-correct desktop download url', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      const previousTauri = globalWindow.__TAURI__;
      const releaseUrl = 'https://github.com/hacck3y/threatatlas/releases/latest';
    ...
    expect(result.macArm).toBe('https://threatatlas.app/api/download?platform=macos-arm64&variant=full');
    expect(result.windowsX64).toBe('https://threatatlas.app/api/download?platform=windows-exe&variant=full');
    expect(result.linuxFallback).toBe('https://github.com/hacck3y/threatatlas/releases/latest');
  });
  ...
  test('cloud fallback blocked without WorldMonitor API key', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      try {
        ...
        const cloudCalls = calls.filter(u => u.includes('threatatlas.app'));
  ...
  test('cloud fallback allowed with valid WorldMonitor API key', async ({ page }) => {
    ...
    const result = await page.evaluate(async () => {
      ...
      window.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes('threatatlas.app') && init?.headers) {
          const h = new Headers(init.headers);
          const wmKey = h.get('X-ThreatAtlas-Key');
          if (wmKey) capturedHeaders['X-ThreatAtlas-Key'] = wmKey;
        }

        if (url.includes('127.0.0.1:46123/api/market/v1/test')) {
          throw new Error('ECONNREFUSED');
        }
        if (url.includes('threatatlas.app/api/market/v1/test')) {
      ...
      const testKey = 'ta_test_key_1234567890abcdef';
      await runtimeConfig.setSecretValue('THREATATLAS_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, testKey);
      ...
      try {
        ...
        return {
          ...
          cloudCalls: calls.filter(u => u.includes('threatatlas.app')).length,
          taKeyHeader: capturedHeaders['X-ThreatAtlas-Key'] || null,
        };
      } finally {
        window.fetch = originalFetch;
        delete globalWindow.__wmFetchPatched;
        ...
        await runtimeConfig.setSecretValue('THREATATLAS_API_KEY' as import('/src/services/runtime-config.ts').RuntimeSecretKey, '');
      }
    });
    ...
    expect(result.taKeyHeader).toBe('ta_test_key_1234567890abcdef');
  });
