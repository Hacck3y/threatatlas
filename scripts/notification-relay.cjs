...
function ensureVapidConfigured(client) {
  ...
  const subject = process.env.VAPID_SUBJECT || 'mailto:support@threatatlas.app';
  if (!pub || !priv) {
    if (!webpushConfigWarned) {
...
async function sendWebPush(userId, subscription, payload) {
  ...
  const body = JSON.stringify({
    ...
    body: payload.body || '',
    url: payload.url || 'https://threatatlas.app/',
...
// backward-compat reading but is no longer written.
    ...
    for (const ch of verifiedChannels) {
      try {
        if (ch.channelType === 'telegram' && ch.chatId) {
          ...
          const eventUrl = event.payload?.link || event.payload?.url || 'https://threatatlas.app/';
