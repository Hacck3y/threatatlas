...
- CDN-Cache-Control header now only set for trusted origins (threatatlas.app, Vercel previews, Tauri); no-origin server-side requests always reach the edge function so `validateApiKey` can run, closing a potential cache-bypass path for external scrapers
...
- **Edge caching**: Cloudflare edge caching for proxy.threatatlas.app (#478) and api.threatatlas.app (#471)
...
- **Finance variant**: Added a dedicated market-first variant (`finance.threatatlas.app`) with finance/trading-focused feeds, panels, and map defaults
