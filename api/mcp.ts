...
const TOOL_REGISTRY: ToolDef[] = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    _execute: async (params, base, apiKey) => {
      const UA = 'threatatlas-mcp-edge/1.0';
      ...
      const digestRes = await fetch(`${base}/api/news/v1/list-feed-digest?variant=geo&lang=en`, {
        headers: { 'X-ThreatAtlas-Key': apiKey, 'User-Agent': UA },
      ...
      const briefRes = await fetch(`${base}/api/news/v1/summarize-article`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-ThreatAtlas-Key': apiKey, 'User-Agent': UA },
        body: JSON.stringify({
          provider: 'openrouter',
          headlines,
          mode: 'brief',
  ...
  },
  {
    ...
    _execute: async (params, base, apiKey) => {
      ...
      const res = await fetch(
        ...
        {
          headers: { 'X-ThreatAtlas-Key': apiKey, 'User-Agent': 'threatatlas-mcp-edge/1.0' },
  ...
  },
  {
    ...
    _execute: async (params, base, apiKey) => {
      ...
      const UA = 'threatatlas-mcp-edge/1.0';
      const headers = { 'X-ThreatAtlas-Key': apiKey, 'User-Agent': UA };
  ...
  },
  {
    ...
    _execute: async (params, base, apiKey) => {
      ...
      const [sw_lat, sw_lon, ne_lat, ne_lon] = bbox;
      const bboxQ = `sw_lat=${sw_lat}&sw_lon=${sw_lon}&ne_lat=${ne_lat}&ne_lon=${ne_lon}`;
      const headers = { 'X-ThreatAtlas-Key': apiKey, 'User-Agent': 'threatatlas-mcp-edge/1.0' };
  ...
  },
  {
    ...
    _execute: async (params, base, apiKey) => {
      const res = await fetch(`${base}/api/intelligence/v1/deduct-situation`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'X-ThreatAtlas-Key': apiKey, 'User-Agent': 'threatatlas-mcp-edge/1.0' },
...
export default async function handler(req: Request): Promise<Response> {
  ...
  //   2. X-ThreatAtlas-Key header — direct API key (curl, custom integrations)
  let apiKey = '';
  ...
  if (authHeader.startsWith('Bearer ')) {
    ...
    if (bearerApiKey) {
      ...
      return new Response(
        ...
        { status: 401, headers: { 'Content-Type': 'application/json', 'WWW-Authenticate': 'Bearer realm="threatatlas", error="invalid_token", resource_metadata="https://api.worldmonitor.app/.well-known/oauth-protected-resource"', ...corsHeaders } }
      );
    }
  } else {
    const candidateKey = req.headers.get('X-ThreatAtlas-Key') ?? '';
    if (!candidateKey) {
      return new Response(
        JSON.stringify({ jsonrpc: '2.0', id: null, error: { code: -32001, message: 'Authentication required. Use OAuth (/oauth/token) or pass your API key via X-ThreatAtlas-Key header.' } }),
        { status: 401, headers: { 'Content-Type': 'application/json', 'WWW-Authenticate': 'Bearer realm="threatatlas", resource_metadata="https://api.worldmonitor.app/.well-known/oauth-protected-resource"', ...corsHeaders } }
      );
    }
    const validKeys = (process.env.THREATATLAS_VALID_KEYS || '').split(',').filter(Boolean);
