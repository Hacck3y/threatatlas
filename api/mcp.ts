  ...
  if (authHeader.startsWith('Bearer ')) {
    ...
    if (bearerApiKey) {
      ...
      return new Response(
        ...
        { status: 401, headers: { 'Content-Type': 'application/json', 'WWW-Authenticate': 'Bearer realm="threatatlas", error="invalid_token", resource_metadata="https://api.threatatlas.app/.well-known/oauth-protected-resource"', ...corsHeaders } }
      );
    }
  } else {
    ...
    if (!candidateKey) {
      return new Response(
        ...
        { status: 401, headers: { 'Content-Type': 'application/json', 'WWW-Authenticate': 'Bearer realm="threatatlas", resource_metadata="https://api.threatatlas.app/.well-known/oauth-protected-resource"', ...corsHeaders } }
      );
    }
