...
function makeRequest(query = '') {
  return new Request(`https://threatatlas.app/api/youtube/embed${query}`);
}
...
test('returns embeddable html for valid video id', async () => {
  ...
  assert.equal(html.includes('origin:"https://threatatlas.app"'), true);
...
test('uses dedicated parentOrigin for iframe postMessage target', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://threatatlas.app&parentOrigin=https://tauri.localhost'));
  const html = await response.text();
...
test('does not accept wildcard parentOrigin query parameter', async () => {
  const response = await handler(makeRequest('?videoId=iEpJwprxDdk&origin=https://threatatlas.app&parentOrigin=*'));
  const html = await response.text();
