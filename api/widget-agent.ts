 ...
 *   2. Browser tester key (X-ThreatAtlas-Key) — validated against
...
const RELAY_BASE = 'https://proxy.threatatlas.app';
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === 'OPTIONS') {
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-ThreatAtlas-Key, X-Api-Key, X-Widget-Key, X-Pro-Key',
      },
    });
  }
  ...
  const worldMonitorKey =
    req.headers.get('X-ThreatAtlas-Key') ??
  ...
  const relayHeaders: Record<string, string> = {
    ...
    'User-Agent': 'threatatlas-widget-edge/1.0',
